component {
	...
	function configure(){
		settings = { key : "", baseUrl : "https://api.mollie.com/v2" };
	}

	function onLoad(){
		binder.map( "mollie@molliecfml" ).to( "#moduleMapping#.mollie" ).asSingleton().initWith( key = settings.key, baseUrl = settings.baseUrl );
	}
